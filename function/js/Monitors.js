...
document.addEventListener('DOMContentLoaded', function() {
    // ล้าง window. ทุกตัวก่อนใช้งาน (แต่ไม่ลบ productTypeMapping และ currentSelectedProductKey)
    ...
    // เก็บ productTypeMapping ไว้
    if (!('productTypeMapping' in window)) {
        window.productTypeMapping = {
            'MAOPN': 'MAOPN&OS3',
            'PIRSSR': 'PIRSSR', 
            'MAWTR-CASE': 'CASE',
            'MAWTR-DENSO24CY': 'DENSO24CY',
            'MAWTR-DENSO & MF': 'DENSO&MF',
            'MAWTR-ELEMENT': 'ELEMENT',
            'MAWTR-E-JAM': 'EJAM',
            'MAWTR-AF-SRT': 'AFSRT',
            'MAWTR-AF-STD': 'AFSTD',
            'MAWTR-AF-VE': 'AFVE'
    ...
    // ใช้ Product Type ที่เลือกจาก filter
    window.selectedProductType = null; // ไม่มีค่าเริ่มต้น รอเลือกก่อน
    
    // ล้างข้อมูลเก่าทั้งหมดเพื่อป้องกันการแสดงข้อมูลเก่า
    window.monitorData = null;
    window.wipData = null;
    window.statusData = null;
    window.downtimeData = null;
    ...
    function updateProductTypeDisplay() {
        ...
        if (productTypeContainer && productTypeSelect) {
            // ใช้ Product Type ที่เลือกจาก filter
            const currentProductType = window.selectedProductType;
            
            if (currentProductType) {
                // อัปเดต select dropdown ให้เลือก Product Type ที่ถูกต้อง
    ...
    // เพิ่ม event listener สำหรับ select dropdown
    const productTypeSelect = document.getElementById('product-type-select');
    if (productTypeSelect) {
        productTypeSelect.addEventListener('change', function() {
            const selectedValue = this.value;
            if (selectedValue) {
                window.changeProductType(selectedValue);
            }
        });
    }
    
    // ฟังก์ชันสำหรับเปลี่ยน Product Type จาก filter
    window.changeProductType = function(newProductType) {
        window.selectedProductType = newProductType;
        ...
        // ล้าง intervals
        if (window.wipStatusLabelInterval) {
            clearInterval(window.wipStatusLabelInterval);
            delete window.wipStatusLabelInterval;
        }
        ...
        // ล้างการแสดงผลเก่า
        const containers = ['#daily-input', '#daily-output', '#acc-input', '#acc-output'];
        containers.forEach(container => {
            const element = document.querySelector(container);
            if (element) {
                element.innerHTML = '<div style="text-align: center; color: #fff; font-size: 14px;">Loading...</div>';
            }
        });
        
        // ล้าง charts containers
        const chartContainers = ['wipStatusChart', 'wipLineChart', 'totalWipChart'];
    ...
    // แสดงข้อความรอเลือก Product Type
    updateProductTypeDisplay();
    
    // ตั้งเวลาให้รีเฟรชข้อมูลใหม่ทุก 60 วินาที (ไม่เปลี่ยน Product Type)
    setInterval(() => {
        console.log('Auto-refreshing data...');
        
        // ตรวจสอบว่ามี Product Type หรือไม่
        if (!window.selectedProductType) {
            console.log('No Product Type selected, skipping auto-refresh');
            updateProductTypeDisplay();
            return;
        }
        
        // อัปเดต Product Type Display
        updateProductTypeDisplay();
        ...
        // โหลดข้อมูลใหม่ด้วย Product Type เดิม
        Load_Monitor_Data();
        Load_WIP_Data();
        Load_Machine_Status_Data();
        Load_McRecord_Data();
...
function Load_Monitor_Data(){
    // ใช้ Product Type ที่เลือกจาก filter
    const productType = window.selectedProductType;
    
    // ตรวจสอบว่ามี Product Type หรือไม่
    if (!productType) {
        console.log('No Product Type selected, skipping data load');
        return;
    }
    ...
    fetch('./function/[Dashboard][Monitors]Data.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function Load_WIP_Data(){
    ...
    // ตรวจสอบว่ามี Product Type หรือไม่
    if (!productType) {
        console.log('No Product Type selected, skipping WIP data load');
        return;
    }
    ...
    fetch(`./function/[Dashboard][WIP]Data.php?${params.toString()}`, {
        method: 'GET'
    })
    .then(response => {
        if (!response.ok) {
...
function Load_Machine_Status_Data(){
    ...
    // ตรวจสอบว่ามี Product Type หรือไม่
    if (!productType) {
        console.log('No Product Type selected, skipping Status data load');
        return;
    }
    ...
    fetch('./function/[Dashboard][Status]Data.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function Load_McRecord_Data(){
    ...
    // ตรวจสอบว่ามี Product Type หรือไม่
    if (!productType) {
        console.log('No Product Type selected, skipping MCRecord data load');
        return;
    }
    ...
    fetch('./function/[Dashboard][MCRecord]Data.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function Update_Last_Refresh(serverTime) {
    ...
    // ดึง Product Type ปัจจุบัน
    const currentProductType = window.selectedProductType || 'No Product Type Selected';
...
function Cal_Acc_Data(data, planData, planType, wipData = 0) {
    ...
    // ดึง productType จาก select dropdown
    const productTypeSelect = document.getElementById('product-type-select');
    const productType = productTypeSelect;
...
function WIPLastMonth(wipData) {
    const wipLastMonthElement = document.querySelector('#wip-last-month');
    if (wipLastMonthElement) {
        const wipValue = parseInt(wipData) || 0;
        wipLastMonthElement.innerHTML = `
            <div style="display: flex; flex-direction: column; height: 100%; justify-content: center; align-items: center; text-align: center;">
                <div class="simple-title">
                    WIP last month: ${wipValue}
                </div>
            </div>
        `;
    }
...
function Machine_Status_Line(data) {
    const group = {};
    data = data.filter(row => row.LINE !== null);
    data.forEach(row => {
...
function Line_Loss_Time(data) {
    ...
    let filteredData = data.filter(row => row.LOSSCODE === 'WR');
...
function FilterDaily(inputData, outputData, planData) {
    // ดึง productType จาก select dropdown
    const productTypeSelect = document.getElementById('product-type-select');
    const productType = productTypeSelect ? productTypeSelect.value : 'MAOPN&OS3';
...
function Show() {
    // ตรวจสอบว่ามี Product Type หรือไม่
    if (!window.selectedProductType) {
        console.log('No Product Type selected, skipping Show()');
        return;
    }
    
    // ตรวจสอบว่าข้อมูลครบหรือยัง
    if (!window.monitorData || !window.wipData || !window.statusData || !window.downtimeData) {
        console.log('Data not complete yet, skipping Show()');
        return;
    }
    ...
    WIPLastMonth(window.monitorData.WIPData || 0);
